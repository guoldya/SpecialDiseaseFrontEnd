<template>
  <!-- <div class="avatar" :class="shape === 'circle' ? 'shape-circle' : ''">
    <img :src="avatarSrc">
  </div> -->
  <div >
      <img class="online-content-list-head"  :src="avatarSrc"   alt>
  </div>
</template>

<script>
import systemAvatar from '@/assets/image/system.png'

export default {
  props: {
    src: String,
    type: {
      type: String,
      default: 'C2C'
    },
    shape: {
      type: String,
      default: 'circle'
    }
  },
  computed: {
    avatarSrc: function () {
      let src = this.src
      if (/^(https:|http:|\/\/)/.test(src)) {
        return src
      } else {
        return this.defaultSrc
      }
    },
    defaultSrc: function () {
      switch (this.type) {
        case 'C2C':
          // 个人头像
          return 'http://thirdwx.qlogo.cn/mmopen/vi_32/LG3SWVib5iaTP5XvnJFVIX5KVaic4oeGmyyJqrExMHZIHWBCJSEzxDTpMHrkwE1kQbxtSOT41w8SrY52IDvvibXUbg/132'
        case 'GROUP':
          // 群默认头像
          return 'https://imgcache.qq.com/open/qcloud/video/act/webim-avatar/avatar-3.png'
        case this.TIM.TYPES.CONV_SYSTEM:
          return systemAvatar
        default:
          // 默认头像
          return 'https://imgcache.qq.com/open/qcloud/video/act/webim-avatar/avatar-1.png'
      }
    }
  }
}
</script>

<style lang="less" scoped>
.avatar {
  background-color: #a5b5c1;
  text-align: center;
  width: 100%;
  height: 100%;
  overflow: hidden;
 
 
  img {
    width: 100%;
    height: 100%;
      width: 90px;
  height: 90px;
    border-radius: 50%;
  }
}

.shape-circle {
  border-radius: 50%;
}
 .online-content-list-head {
        width: 90px;
        height: 90px;
        border-radius: 50%;
      }
</style>
